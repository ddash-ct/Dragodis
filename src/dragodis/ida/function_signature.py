...
from dragodis.exceptions import NotExistError, UnsupportedError, BackendError
...
class IDAFunctionSignature(FunctionSignature):
    ...
    def return_type(self) -> IDADataType:
        rettype = self._func_type_data.rettype
        if self._ida.ida_version >= 850:
            # Make copy to prevent unexpected mutations.
            rettype = rettype.copy()
        return IDADataType(self._ida, rettype)
    ...
    def replace_parameters(self, data_types: List[str]):
        ...
        success = ida_typeinf.parse_decl(tif, til, declaration, ida_typeinf.PT_SIL)
        if not success:
            raise BackendError(f"Failed to parse declaration: {declaration}")
    ...
    def add_parameter(self, data_type: str) -> IDAFunctionParameter:
        ...
        new_param.type = IDADataType._create_tinfo(self._ida, data_type)
