...
from dragodis.interface.operand_value import OperandValue
from dragodis.ida.operand_value import (
    IDARegister, IDAImmediate, IDAMemoryReference,
        ...
        if isinstance(value, Phrase):
            ...
            if isinstance(offset, int):
                if self._ida.ida_version < 850:
                    stack_var = self._ida._ida_frame.get_stkvar(self.instruction._insn_t, self._op_t, offset)
                    if stack_var:
                        member, _ = stack_var
                        frame = self._ida._ida_frame.get_frame(self.address)
                        frame = IDAStackFrame(self._ida, frame)
                        return IDAStackVariable(self._ida, frame, member)
                else:
                    #Stack API was changed a ton in IDA 8.5
                    frame_id = self._ida._idc.get_frame_id(self.address)
                    if frame_id != self._ida._BADADDR:
                        frame = self._ida._ida_typeinf.tinfo_t(tid=frame_id)
                        index = frame.get_stkvar(self.instruction._insn_t, self._op_t, offset)
                        if index != -1:
                            index, member = frame.get_udm(index)
                            if index != -1:
                                frame = IDA9StackFrame(self._ida, frame)
                                return IDA9StackVariable(self._ida, frame, member, index)
...
class IDAx86Operand(IDAOperand, x86Operand):
    ...
    def type(self) -> OperandType:
        ...
        if type == OperandType.memory:
            try:
                # Check if we have an index register to know if this should
                # be a phrase.
