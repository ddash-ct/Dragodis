...
class GhidraLine(Line):
    ...
    def type(self, new_type: LineType):
        ...
        if new_type == LineType.unloaded:
            ...
            raise UnsupportedError(f"Setting a line type to unloaded is not currently supported.")
    ...
    def _set_value(self, new_value: Any):
        ...
        elif type_ == LineType.dword or (type_ == LineType.pointer and self._ghidra.bit_size == 32):
            self._ghidra._flatapi.setInt(self._addr_obj, JInt(new_value))

        elif type_ == LineType.qword or (type_ == LineType.pointer and self._ghidra.bit_size == 64):
