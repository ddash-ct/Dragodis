...
class FlowType(IntEnum):
    call = auto()
    # TODO: 'terminal' accounts for both subroutine returns and interrupts.
    #   Should we make the distinction with separate types?
    terminal = auto()
...
class OperandType(IntEnum):
    ...
    memory = auto()         # Direct Memory Reference
...
class ReferenceType(IntEnum):
    ...
    def is_data(self):
        return self in (self.data, self.data_offset, self.data_write, self.data_text, self.data_informational)

    def is_code(self):
        return self in (self.code_call, self.code_jump, self.code_user, self.ordinary_flow)
...
class CompilerType(Enum):
    visual_c_plus_plus = auto()
    borland_c_plus_plus = auto()
    gcc = auto()
    delphi = auto()
...
COMPILER_VISUAL_C_PLUS_PLUS = CompilerType.visual_c_plus_plus
COMPILER_BORLAND_C_PLUS_PLUS = CompilerType.borland_c_plus_plus
COMPILER_GCC = CompilerType.gcc
COMPILER_DELPHI = CompilerType.delphi

FILETYPE_BINARY = FileType.BINARY
FILETYPE_PE = FileType.PE
FILETYPE_ELF = FileType.ELF
FILETYPE_MACH_O = FileType.MACH_O
