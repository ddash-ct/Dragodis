...
from dragodis.interface.types import OperandType, CommentType, CompilerType, ReferenceType
...
class FlatAPI(metaclass=abc.ABCMeta):
    ...
    def version(self) -> str:
        """
        Returns the version of the underlying disassembler.
        """
    ...
    def project_path(self) -> Path:
        """
        Location of saved project file(s).
        """
    ...
    def is_x86(self) -> bool:
        """
        Returns true if the processor is an X86 processor.
        """
        return self.processor_name in ("x86", "i386", "amd64")

    @property
    def is_arm(self) -> bool:
        """
        Returns true if the processor is an ARM processor.
        """
        return self.processor_name.lower().startswith("arm")
    ...
    def byteorder(self) -> str:
        """
        The byte order as a string, useful for from_bytes()/to_bytes() conversions.
        """
    ...
    def entry_point(self) -> Optional[int]:
        ...
        :returns: Address of the entry point or None if not applicable.
        """
    ...
    def set_compiler(self, compiler):
        """
        Changes the compiler in the disassembler to the specified compiler.

        :param compiler: The compiler to set in the disassembler.

        :raises RuntimeError: If the compiler in the disassembler is not changed to the specified compiler.
        """

    @abc.abstractmethod
    def set_bytes(self, addr: int, data: bytes):
        """
        Sets the raw bytes at the indicated address.

        :param addr: Address to write data.
        :param data: Data to write.

        :raises NotExistError: If the given address is invalid.
        """
