...
class Instruction(metaclass=abc.ABCMeta):
    ...
    def __new__(cls, api: FlatAPI, *args, **kwargs):
        ...
        """
        klass = cls._get_class(api)
        self = super().__new__(klass)
        self.__init__(api, *args, **kwargs)
        return self

    @classmethod
    def _get_class(cls, api: FlatAPI) -> Type[Instruction]:
        """
        Determines the instruction class based on the detected processor.
        """
        if api.is_arm:
    ...
    def flow_type(self) -> FlowType:
        """
        The type of code flow for the instruction.
        """
