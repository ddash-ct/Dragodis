...
def main():
    parser = argparse.ArgumentParser("dragodis.config", description="Initializes and opens settings.toml in user directory.")
    ...
    if args.command == "create":
        ...
        if user_config.exists() and not args.overwrite:
            sys.exit("Error: settings file already exists and --overwrite flag was not specified.")
        _create_config(path)

    elif args.command == "list":
        subprocess.run(["dynaconf", "-i", "dragodis.config.settings", "list"])
